...
reformat.sh in="${ASSEMBLY_OUTPUT}/contigs.fasta" out="${ASSEMBLY_OUTPUT}/filtered_contigs.fasta" minlength=1000

#BWA alignment reads to contigs
bwa index "${ASSEMBLY_OUTPUT}/filtered_contigs.fasta"
bwa mem -t ${coreNum} "${ASSEMBLY_OUTPUT}/filtered_contigs.fasta" "${QC_FASTQ}/read1_trimmed.fastq.gz" "${QC_FASTQ}/read2_trimmed.fastq.gz" | samtools sort -@ ${coreNum} -o "${ALIGNMENT_OUTPUT}/reads_aligned_contigs.sorted.bam" -
